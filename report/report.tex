...
In each image, around 1000 to 3000 features are found.
...
using its depth map values, and reprojected into image $j$ (see figure \ref{fig:reg}). The minimisation
...
pose estimates to improve accuracy. In particular, the registration of a camera pair (i,j) is considered successful, only when the rotational vector $r_{ij}$ and $r_{ji}$ from the bi-directional registration are anti-parallel and have similar magnitude.
